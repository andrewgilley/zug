...
const GenerationContext = struct {
    ...
    fn messageFieldNeedsPointer(_: *GenerationContext, current_fqn: FullName, field: descriptor.FieldDescriptorProto) bool {
        const type_name = field.type_name orelse return false;
        const dep_name = if (std.mem.startsWith(u8, type_name, ".")) type_name[1..] else type_name;

        var maybe_parent: ?FullName = current_fqn;
        while (maybe_parent) |parent| {
            if (parent.eqlString(dep_name)) return true;
    ...
    fn getFieldType(
        ...
        if (!repeated) {
            if (!is_union) {
                ...
                switch (t) {
                    .TYPE_MESSAGE => {
                        prefix = if (self.messageFieldNeedsPointer(fqn, field)) "?*" else "?";
                    },
